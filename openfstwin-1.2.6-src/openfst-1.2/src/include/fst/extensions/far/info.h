...
void FarInfo(const vector<string> &filenames, const string &begin_key,
  ...
  if (!infos) {
    ...
    for (size_t i = 0; i < infos->size(); ++i) {
      ...
      if (ceil(log10((float)info.nstate)) + 2 > wnstate) //Changed Paul Dixon 04/09/10
        wnstate = ceil(log10((float)info.nstate)) + 2; //Changed Paul Dixon 04/09/10
      if (ceil(log10((float)info.narc)) + 2 > wnarc)
        wnarc = ceil(log10((float)info.narc)) + 2; //Changed Paul Dixon 04/09/10
    }
