...
class FrontEnd:
    ...
    def pre_emphasize(self, wav):
        ...
        preemph_wav = np.zeros(wav.shape, dtype=wav.dtype)
        preemph_wav[0]=wav[0]
        preemph_wav[1:] = wav[1:] - self.preemphasis * wav[0:len(wav)-1]        
        return preemph_wav
    ...
    def frames_to_magspec(self, frames):
        frames_fft = np.zeros([self.fft_size,frames.shape[1]])
        for t in range(0,frames.shape[1]):
            frames_fft[:,t] = abs(np.fft.fft(frames[:,t], self.fft_size ))
        frames_fft = ((1.0 / self.fft_size) * ((frames_fft) ** 2)) 
        magspec = frames_fft[0:int(self.fft_size/2 + 1),:]
        magspec = magspec.transpose()
        return magspec
    ...
    def magspec_to_fbank(self, magspec):
        fbank = np.dot(magspec,self.mel_filterbank.transpose())
    ...
    def mean_norm_fbank(self, fbank):
            mean_value = np.mean(fbank)
            norm_fbank = fbank - mean_value
            return norm_fbank.transpose()
